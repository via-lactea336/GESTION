...
export default function Pagination({ changeIndicePagina, indiceActual, indicesPagina }:Prop) {
    ...
    for (let i = Math.max(2, current - delta); i <= Math.min(totalIndices - 1, current + delta); i++) {
      range.push(i);
    }
            ...
            <button
              ...
              className={
                (typeof(page) === 'number' && page - 1 === current
                  ...
                  : typeof(page) === 'string'? 'opacity-50 cursor-not-allowed '
