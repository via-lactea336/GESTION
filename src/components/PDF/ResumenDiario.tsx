            ...
            {movimientos?.map((mov) => (
                <View style={styles.movimientoSection} key={mov.id}>
                    ...
                    <View style={styles.movimientoDetails}>
                        <View>
                            <Text style={[styles.detalle, styles.textContainer]}>Monto total de la operacion: {Number(mov.monto).toLocaleString()} Gs.</Text>
                            <Text style={[styles.detalle, styles.textContainer]}>Fecha: {formatDate(mov.createdAt)}</Text>
                            <Text style={[styles.detalle, styles.textContainer]}>Hora: {formatTime(mov.createdAt)}</Text>
                        </View>
                        <View>
                            {mov.esIngreso ? (
                                <>
                                    <Text style={[styles.detalle, styles.textContainer]}>Detalles de Factura</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>Tipo de factura: {mov.factura?.esContado ? "Contado" : "Credito"}</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>Numero de Factura: {mov.factura?.numeroFactura}</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>Fecha de emision de la factura: {mov.factura?.createdAt ? formatDate(mov.factura.createdAt) : ""}</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>Monto total a pagar: {Number(mov.factura?.total).toLocaleString()} Gs.</Text>
                                </>
                            ) : (
                                <>
                                    <Text style={[styles.detalle, styles.textContainer]}>Encargado: {mov.comprobante?.user?.nombre} {mov.comprobante?.user?.apellido}</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>RUC: {mov.comprobante?.user?.docIdentidad}</Text>
                                    <Text style={[styles.detalle, styles.textContainer]}>Observaciones: {mov.comprobante?.concepto}</Text>
                                </>
                            )}
                        </View>
                    </View>
                    {mov.esIngreso && mov.movimientoDetalles?.length > 0 && (
                        <View>
                            <Text style={[styles.detalle, styles.textContainer]}>Detalles Operacion</Text>
                            {mov.movimientoDetalles.map((detalle, index) => (
                                <View key={index} style={[styles.tableRow, { borderBottom: '1px solid #000' }]}>
                                    <Text style={styles.tableCell}>N°{index + 1}</Text>
                                    <Text style={styles.tableCell}>Metodo de pago: {detalle.metodoPago}</Text>
                                    <Text style={styles.tableCell}>Monto Parcial: {Number(detalle.monto).toLocaleString()} Gs.</Text>
                                </View>
                            ))}
                        </View>
                    )}
