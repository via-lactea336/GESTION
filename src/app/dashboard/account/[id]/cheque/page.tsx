...
import { ArrowDownLeftIcon, ArrowUpRightIcon, MagnifyingGlassIcon, TrashIcon } from '@heroicons/react/24/solid'
...
export default function Cheque({ params }: { params: { id: string } }) {
  ...
  const handleAnular = async (id: string, bancoAfectadoId: string) => {
    ...
    const prevCheques = [...cheques]
    setCheques(prevCheques.map(cheque => cheque.id === id ? {...cheque, estado: estadoCheque.ANULADO} : cheque))
  ...
  return (
    <div className="flex flex-col h-full -mt-8">
      <header className="flex gap-3 justify-between items-center flex-wrap px-8 py-4 w-full rounded-md bg-primary-800 text-white">
          ...
          <div className='flex flex-col items-center gap-3'>
              <div className="flex items-center gap-3">
                ...
                <input 
                  ...
                  className='bg-gray-800 text-white py-1 px-2 rounded' placeholder='Fecha Desde' type="date" 
                  name="fechaDesde" id="fechaDesde" 
                />
              </div>
              <div className='flex items-center gap-3'>
                ...
                <input 
                  ...
                  className='bg-gray-800 text-white py-1 px-2 rounded' placeholder='Fecha Hasta' type="date" 
        ...
        <Link className='bg-gray-800 hover:bg-gray-900 text-white  py-2 px-4 rounded'  href={`/dashboard/account/${id}`} >Atras</Link>
      </header>
      ...
      <div className="flex-grow bg-gray-800 rounded-md p-5 flex flex-col">
        {
          ...
          <>
            <table className="border-collapse w-full">
              <thead>
                <tr>
                  ...
                  <th className='text-left' ><span className='text-md mt-1 text-primary-400 font-normal'>N° Cheque</span></th>
                  <th className='text-left'><span className='text-md mt-1 text-primary-400 font-normal'>Fecha</span></th>
                  <th className='text-left'><span className='text-md mt-1 text-primary-400 font-normal'>Cheque Banco</span></th>
                  <th><span className='text-md mt-1 text-primary-400 font-normal'>Estado</span></th>
                  <th className='text-left'><span className='text-md mt-1 text-primary-400 font-normal'>Monto</span></th>
              ...
              <tbody>
                {
                  cheques.map((cheque) => (
                    <tr className='border-b-2 border-b-gray-700 text-sm' key={cheque.id}>
                      ...
                      <td className='flex justify-center'>{
                        cheque.estado === estadoCheque.PAGADO ? 
                          <span className='bg-green-500 p-1 rounded my-2'>{estadoCheque.PAGADO}</span> 
                        : cheque.estado === estadoCheque.EMITIDO ? 
                          <span className='bg-yellow-500 p-1 rounded my-2'>{estadoCheque.EMITIDO}</span>
                        :
                          <span className='bg-red-500 p-1 rounded my-2'>{estadoCheque.ANULADO}</span>
                        }</td>
            ...
            <div className="mt-2 w-full">
              <Pagination changeIndicePagina={changeIndicePagina} indiceActual={indiceActual} indicesPagina={indicesPagina} />
            </div>
          </>
          )
        }
