...
export async function POST(req: NextRequest) {
  ...
  if( !clienteId || !facturaId || !totalPagado ) return generateApiErrorResponse("Faltan datos para la creacion de el recibo", 400)
    
  if(totalPagado.lessThanOrEqualTo(0)) return generateApiErrorResponse("El monto del recibo no puede ser menor o igual a 0", 400)
  ...
  if(!verifyFactura) return generateApiErrorResponse("La factura a la que se hace referencia no existe", 404)
  if(verifyFactura.totalSaldoPagado.add(totalPagado).greaterThan(verifyFactura.total)) return generateApiErrorResponse("El total que el cliente pagara no puede ser mayor que el total de la factura", 400)
  ...
  try{
    ...
    if(!updateFactura) return generateApiErrorResponse("Error actualizando el total de la factura", 400)
